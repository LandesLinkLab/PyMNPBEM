...
For a sphere on a glass substrate, use the layer structure classes
(LayerStructure, CompGreenStatLayer, BEMStatLayer) -- see
examples/03_substrate_layered.py.
"""
...
def drude_gold():
    ...
    """
    # EpsDrude takes the plasma frequency and damping rate in eV.
    eps_inf = 9.5       # High-frequency dielectric constant
    omega_p = 8.95      # Plasma frequency (eV)
    gamma = 0.069       # Damping rate (eV)

    return EpsDrude(eps_inf, omega_p, gamma)
...
def calculate_spectrum_quasistatic(radius=10.0, n_faces=144, wavelengths=None):
    ...
    for i, enei in enumerate(wavelengths):
        ...
        sig, bem = bem.solve(exc_dict)

        # Calculate cross sections. The quasistatic routines return a bare
        # float for a single polarization, an ndarray otherwise.
        sca[i] = np.ravel(exc.scattering(sig))[0]
        abs_cs[i] = np.ravel(exc.absorption(sig))[0]
        ext[i] = np.ravel(exc.extinction(sig))[0]
...
For particles on a substrate (e.g., glass), use the layer structure
classes -- see examples/03_substrate_layered.py.
