...
Choosing option 3 also keeps the existing complex128 numba path intact:
...
```
mnpbem/greenfun/
  hmatrix.py            # CPU H-matrix (unchanged)
...
3. Add a 25 k face benchmark (sphere or rod) and wire a regression gate
   `gpu_fill < 1.5 * cpu_fill` once break-even is crossed.
