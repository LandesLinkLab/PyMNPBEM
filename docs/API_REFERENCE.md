...
|---|---|---|
| `trisphere(n, diameter=1.0)` | `n : int` (number of **vertices**) | Triangulated sphere from a precomputed mesh. `n` snaps to the closest available value in `{32, 60, 144, 169, 225, ..., 1225, 1444}`. A closed triangulation of `n` vertices has `2n - 4` boundary elements, so `trisphere(144, 20).n == 284`. |
...
`hmatrix=True` option of `BEMStatIter` / `BEMRetIter` enters the *full
H-tree* path rather than block-level ACA.
...
`hmatrix=False` (default): the dense or block-ACA path.
...
outside GMRES → solve the reduced problem. In this operator form the Schur
reduction and the H-matrix iterative path can be activated together.
...
| `PlaneWaveStatMirror(pol, medium=1, **opts)` | + mirror symmetry |
| `PlaneWaveRetMirror(pol, dir, medium=1, **opts)` | + mirror symmetry |
...
Analytical reference solutions for spheres.
...
| Symbol | Description |
|---|---|
| `matmul(a, x)` | Batched 3x3 matrix-matrix multiply over leading dims. |
| `inner(nvec, a, mul=None)`, `outer(nvec, val, mul=None)` | Batched inner / outer products of 3-vectors. |
| `matcross(a, b)` | Cross product. |
| `vec_norm(v, key=None)`, `vec_normalize(v, v2=None, key=None)` | Vector norm / unit. |
| `spdiag(a)` | Sparse diagonal. |

### Distance helpers

| Symbol | Description |
|---|---|
| `pdist2(p1, p2)` | All-pair squared distance (mirrors MATLAB `pdist2`). |
| `bradius(p)` | Bounding-sphere radius of a particle. |
| `bdist2(p1, p2)` | Bounding-sphere distance squared. |
| `distmin3(p, pos, cutoff=None)` | Minimum 3D distance helper. |
...
Because the GPU dependency ships as an extra, this helper checks in
advance, at runtime, whether GPU acceleration can be enabled.
...
raised at the point the BEM solver is called.
