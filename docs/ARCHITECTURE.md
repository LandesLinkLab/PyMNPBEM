...
└── examples/                   # runnable scripts + Jupyter tutorial
```
...
See: `mnpbem/utils/matlab_compat.py`.
...
- Without it the CPU path lags MATLAB on dense meshes.
...
matrix assembly stage be eagerly pushed to GPU.

### 3.9 Why we deviate from MATLAB (intentionally)

| Item | Difference | Reason |
|---|---|---|
| FP roundoff | up to ~ULP in some demos | MATLAB libmwmathutil vs. Python math libs; minimised by `matlab_compat` but not always eliminated |
...
| `EV2NM` constant | `1/8.0655477e-4` | Match MATLAB `Misc/units.m` exactly |
...
|---|---|---|---|
| Wavelength distribution | N | 1 GPU's worth | wavelength distribution |
| **VRAM share** | 1 | sum of M GPUs | large single computation |
...
- Limitations: NVIDIA GPU only (cusolvermg backend), AMD GPU unsupported.
  ...
  even in a 4-GPU pool (192 GB) and must be combined with an H-matrix (§3.13).

### 3.13 H-matrix BEMRetIter integration

Resolves the dense-LU memory limit (50+ GB peak) of large meshes (25k+ face)
with ACA H-matrix + GMRES. Whereas the VRAM share of §3.12 circumvents the GPU
limit by building a *memory pool* for the dense LU, the H-matrix iterative path
...
| dense + VRAM share | `O(N^2)`, multi-GPU pool | `O(N^2)` | 25k face (2 GPU pool) |
| **H-matrix BEMRetIter** | `O(N log N)` | `O(N log N)` per iter | 25k+ face |
...
- The Jacobi preconditioner (`precond='diag'`) and the H-matrix LU
  preconditioner of §3.15 are the compatible choices.

### 3.14 CPU/GPU build layout
...
| `mnpbem[dev]` | pytest / build / twine | development environment |
...
cover-layer variables. The H-matrix BEMRetIter of §3.13 (`O(N log N)`
memory/matvec) solves the "does it solve at all" problem, but the "does it
converge fast enough" problem needs a preconditioner, and the "8N×8N system
with an added cover layer" problem needs the Schur × Iter integration. This
section covers both.
...
- Limitation — in `BEMRetIter`'s 8N×8N coupled system, the G-only H-tree LU
  ...
  work. In `BEMStatIter` tree mode the diagonal term breaks, so it falls back
...
- BEM-solver-side exposure: `BEMRetIter(p, hmatrix=True, schur=True)`.
  Auto-detects the cover layer (reusing `detect_shell_core_partition` from
  the dense Schur path of §3.11).
...
### 3.16 B-Schur coverage and the BEMRetLayerIter operator form
...
`_afun / _init_precond / _mfun` all apply the operator form. Resolves the
drift in the substrate + iter + multi-material case.

## 4. Performance summary

Speed comes from four layers, in increasing order of impact:

- `scipy` LU flags (`check_finite=False` / `overwrite_a=True`): 10-20% LU gain.
- Multi-RHS wavelength batching and the GMRES iterative path.
- Numba kernels: 5-50× on the dense Green fill.
- GPU, H-matrix and FMM3D: order-of-magnitude on large meshes.

Headline numbers, as a geometric mean over the 72 MNPBEM demos against
MATLAB MNPBEM17: **2.21×** on CPU and **3.60×** on a single GPU.
...
(floating point, mesh generation), are summarised in the README.
...
- Adding a BEM solver: extend `BemBase`, mirror the MATLAB `@bem*` class,
  and register it in `mnpbem.bem.solver_factory.solver`.
- Adding an excitation: place it under `mnpbem.simulation`, expose it in
  `mnpbem/simulation/__init__.py`, and register the factory.
