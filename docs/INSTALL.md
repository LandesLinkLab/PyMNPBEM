...
python -c "import mnpbem; print(mnpbem.__version__)"   # smoke check
python examples/01_sphere_extinction.py                # end-to-end check
```

The `dev` extra adds pytest, build, and twine. Combine with `all` to
also pull the GPU / MPI / FMM stacks.
...
| `MNPBEM_MATLAB_LIB` | see note | Path to MATLAB's `libmwmathutil.so`, used only for bit-exact MATLAB comparisons. |

`MNPBEM_MATLAB_LIB` defaults to
`/usr/local/MATLAB/R2025b/bin/glnxa64/libmwmathutil.so`. It is optional: when
the library is not readable, `mnpbem.utils.matlab_compat` falls back to the
numpy equivalent of every function, which is the normal path for users who do
...
p = trisphere(144, 20)                 # 144-vertex sphere, 20 nm diameter
print('vertices =', p.verts.shape[0])  # 144
print('elements =', p.n)               # 284 boundary elements
print('eps(500 nm) =', EpsTable('gold.dat')(500.0)[0])
```

`p.n` is the number of *boundary elements* (faces), not vertices: a closed
triangulation of `V` vertices has `2V - 4` faces.
