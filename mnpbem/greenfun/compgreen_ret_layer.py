...
class CompGreenRetLayer(object):
    ...
    def _assembly(self, g_direct, gr_comp):
        ...
        # Backend consistency: when MNPBEM_GPU_LAYER is active, gr_comp
        # values may be cupy ndarrays while g_direct is a host numpy array
        # (or vice versa when both share the GPU path).  Materialise both
        # sides on the host before the in-place add to avoid a mix in the
