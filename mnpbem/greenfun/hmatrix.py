...
class HMatrix(object):
    ...
    def _aca_block(self,
        ...
        # Ensure rows/cols index arrays live on host (numpy).
        ...
        for k in range(max_rank):
            ...
            # cupy → numpy if a GPU-aware ``fun`` returned cupy.
            ...
            # Explicit int() so cols[pivot_col_local] never hits
    ...
    def full(self, xp: Any = None) -> Any:
        ...
        #
        # When any of ``self.val`` / ``self.lhs`` /
        # ``self.rhs`` lives on a CUDA device (cupy ndarray), the host
        # numpy buffer allocated here would raise ``TypeError: Implicit
        ...
        # GPU for the 12672-face Au@Ag case.
        tree = self.tree
        n = tree.n
        ...
        # Double the working set and OOM the 49 GB A6000 at 12672 faces
    ...
    def mtimes_vec(self, v: np.ndarray) -> np.ndarray:
        ...
        # dtype/backend consistency fix.  Sniff whether any block
        ...
        # plain ndarray (the BEMRetIter call sites assume numpy).
        if on_gpu and not v_is_cupy:
    ...
    def _plus_hmat(self, other: 'HMatrix') -> 'HMatrix':
        ...
        #
        # When one operand has cupy blocks and the
    ...
    def _truncate_block(self,
        ...
        # MATLAB: truncate/fun
        # When blocks live on GPU (cupy) the np.linalg
