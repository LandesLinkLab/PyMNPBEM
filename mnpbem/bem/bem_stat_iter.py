...
# ---------------------------------------------------------------------------
# Quasi-static distributed-build helpers
# ---------------------------------------------------------------------------
...
def _vram_share_distributed_kwargs_stat() -> Dict[str, Any]:
    ...
    VRAM-Schur: mirror of ``bem_stat._vram_share_distributed_kwargs``
...
def _compgreen_stat_eval_block(green: Any,
    ...
    Fallback for ``CompGreenStat.eval_block``, used when the installed
    CompGreenStat predates that method.
...
def _distributed_block_assemble_stat(green: Any,
    ...
    # Prefer the real CompGreenStat.eval_block when available.
    # Its signature is (key, enei, col_start, col_stop) — enei is
    ...
    def _safe_eval(key: str, c0: int, c1: int) -> Any:
        if _have_real_eval_block:
            # Try the 4-arg signature first; if a hypothetical
...
# GPU memory-pool cleanup: mirror BEMRet's wavelength-end immediate free
...
def _gpu_pool_cleanup_stat(apply_limit: bool = False) -> None:
    ...
    Mirrors the BEMRet cleanup (bem_ret.py:485-503, 734-737): the
...
class BEMStatIter(BEMIter):
    ...
    def __init__(self,
        ...
        self._schur_op = None
        # VRAM-Schur: host-side keepalive for the distributed
        ...
        # H-matrix: opt-in ACA acceleration of F. The matvec used
        ...
        # H-matrix LU preconditioner for the
    ...
    def _init_green(self,
        ...
        # when distributed build is active AND we are
    ...
    def _build_distributed_F(self) -> None:
        """build F via distributed column-tile assembly.
    ...
    def _init_matrices(self,
        ...
        # VRAM-Schur: when both distributed-build AND Schur are
        ...
        # wavelength-entry GPU cleanup: drain any stale residents from
        ...
        if self.precond is not None:
            ...
            if hasattr(F, 'full') and not isinstance(F, np.ndarray):
                ...
                F_dense = F
            n = F_dense.shape[0]
            # densification of F (when F is an HMatrix) can leave
            ...
            # ``F_dense`` / ``Lambda`` / their sum are transient
    ...
    def _init_distributed_schur(self,
            enei: float) -> 'BEMStatIter':
        """VRAM-Schur: distributed Schur reduction for the iter path.
    ...
    def solve(self,
        ...
        # drain the cupy pool right before GMRES enters its Krylov
        ...
        if self._schur_active:
            ...
            # VRAM-Schur: when ``self._mat_lu`` carries a reduced LU
            ...
            # LU.  Otherwise the preconditioner is
            ...
            # drop the Schur-reduced Krylov subspace handle and
            ...
            # GMRES holds up to ``restart`` Krylov vectors of length
        ...
        # solve-exit cleanup: free any residual transient buffers
    ...
    def _build_hlu_preconditioner(self,
        ...
        # H-matrix LU preconditioner for BEMStatIter.
        ...
        # as the GMRES preconditioner. This is equivalent to
        # ``precond='hmat'`` but now triggered on the H-matrix code path
        # (where it was previously disabled by default).
        ...
        if self._mat_lu is None:
            ...
            # the densify-and-LU pipeline above can spike GPU
    ...
    def field(self,
        ...
        # drain after the field-side matvec to keep the pool from
        ...
        e = e - phip
        # drain residual transient buffers from the interp/deriv
    ...
    def potential(self,
        ...
        # drain after the Green-function potential evaluation so
    ...
    def clear(self) -> 'BEMStatIter':
        ...
        # Also reset the cache gate (enei) and the wavelength-
        ...
        #
        # VRAM-Schur: if _mat_lu carries an 'mgpu' tag, close the
        ...
        self._schur_op = None
        self._hlu_object = None
        # VRAM-Schur: drop the distributed-Schur keepalive that
    ...
    def __call__(self,
        ...
        # explicit __call__(enei) is a user-driven wavelength step.
