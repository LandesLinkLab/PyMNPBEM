...
# ------------------------------------------------------------------
# The dense path (mnpbem/bem/schur_helpers.py) eliminates the
...
class SchurIterOperator(LinearOperator):
    ...
    # MNPBEM: Schur reduced GMRES operator on top of a
    ...
    #
    # ``eps_form`` communicates whether the upstream ``_afun`` uses
    # operator-form eps.  When ``eps_form='operator'``, the dense A_ss
    # probe is bypassed in favor of an inner GMRES solver because the
    # operator-form full matvec produces an A_ss whose dense probe is
    # ill-conditioned for nonlocal cover-layer geometries.
    ...
    def __init__(self,
        ...
        if g_ss_solver == 'auto':
            # for operator-form eps the inner GMRES path
            ...
            # the eps-apply + extra G/H matvec).  Dense LU probe
...
def detect_iter_partition(particle: Any) -> Optional[Tuple[np.ndarray, np.ndarray]]:
    ...
    # Re-export the partition detector unchanged so callers
...
def schur_iter_memory_estimate(
    ...
    # dense estimator this accounts for the per-face component
...
# ---------------------------------------------------------------------------
# Distributed Schur preconditioner adapter (VRAM-Schur)
# ---------------------------------------------------------------------------
...
def make_distributed_schur_preconditioner(
    ...
    # bem_ret_iter.py is unchanged) could use the same adapter
