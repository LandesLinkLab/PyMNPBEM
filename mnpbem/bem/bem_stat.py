...
# memory-pool parity: BEMStat's quasistatic single-LU pipeline
...
# the entire sweep.  Mirror BEMRet._init_gpu_assemble's pattern:
...
class BEMStat(object):
    ...
    def __init__(self, p, enei=None, **options):
        ...
        **options : dict
        ...
        # VRAM-Schur: keepalive dict for the distributed Schur
    ...
    def _init_matrices(self, enei):
        ...
        if self.enei is None or self.enei != enei:
            # Distributed multi-GPU build path. When MNPBEM_VRAM_SHARE
            ...
            # cuSolverMg LU factor.  VRAM-Schur extends this to
            ...
            # MATLAB-parity: free cupy pools before allocating the
            ...
            # small intermediate sync — when eps1/eps2 evaluation
            ...
            # Lambda is a diag-only N^2 buffer; once added into
            ...
            if self._schur_opt:
                ...
                if partition is not None:
                    ...
                    del M_eff
                    # post-Schur-LU pool drain (the schur_eliminate
                ...
                self.mat_lu = lu_factor_dispatch(M_full, **_lu_opts)
            # drop M_full (now captured by the LU factor) so the
    ...
    def _init_distributed_assemble(self, enei):
        """Distributed multi-GPU quasistatic BEM matrix assembly.
    ...
    def _init_distributed_schur(self, enei):
        """VRAM-Schur: distributed BEM matrix build + Schur reduction.
    ...
    def __truediv__(self, exc):
        ...
        # Host-materialize so np.asarray(sig.sig) works.
    ...
    def _schur_solve(self, phip):
        ...
        # post-Schur-recover pool drain — the recover callable
    ...
    def clear(self):
        ...
        """
        # MATLAB: obj.mat = []
        # Also reset enei so the cache gate in _init_matrices
        ...
        # When the LU is an 'mgpu' tuple we must close the handle
        ...
        self._schur_reduce_rhs = None
        self._schur_recover = None
        # VRAM-Schur: drop the distributed-Schur host keepalive
        ...
        if hasattr(self, '_schur_dist_keepalive'):
            self._schur_dist_keepalive = None
        # an explicit clear() is the user's signal that they want
    ...
    def _lu_solve(lu_piv, b):
        if isinstance(lu_piv, tuple) and len(lu_piv) == 3 and lu_piv[0] in ("cpu", "gpu", "mgpu"):
            ...
            # post-solve pool drain — when lu_piv lives on cupy
