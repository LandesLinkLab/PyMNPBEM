...
# BEMStatMirror dense LU loop now honours the same
...
class BEMStatMirror(object):
    ...
    def __init__(self,
        ...
        # drain the cupy pool after the contracted F-block
    ...
    def _init_matrices(self, enei: float) -> 'BEMStatMirror':
        ...
        # Distributed multi-GPU build path: each per-symmetry F[i]
        ...
        # free any previous wavelength's LU list before
        # allocating new device-resident factors.  Mirrors the BEMStat
        ...
        for i in range(len(self.F)):
            ...
            # drop the per-block M_full handle so its N^2
        ...
        self.enei = enei
        # final pool drain so the next wavelength entry sees
        # a clean device.
        _gpu_pool_cleanup_mirror()
        return self
    ...
    def _init_distributed_assemble(self,
        ...
        """Distributed multi-GPU build for the quasistatic mirror solver.
    ...
    def solve(self, exc: CompStructMirror) -> Tuple[CompStructMirror, 'BEMStatMirror']:
        ...
        for i in range(len(exc.val)):
            ...
            # Host-materialize for user-facing access.
        ...
        # post-solve pool drain (per-symmetry LU back-
    ...
    def clear(self) -> 'BEMStatMirror':
        ...
        API parity with BEMStat / BEMStatLayer / BEMStatIter.
        ...
        Close mgpu handles + free their distributed buffers.
        """
