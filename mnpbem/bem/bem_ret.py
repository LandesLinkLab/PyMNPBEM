...
# When MNPBEM_GPU=1 and cupy is importable, the
...
def _bem_native_gpu() -> bool:
    """Keep BEMRet matrices (Sigma1, L1/L2) on device when set.

    Default ON when MNPBEM_GPU=1 (native path verified bit-identical).
...
class BEMRet(object):
    ...
    def __init__(self, p, enei=None, **options):
        ...
        **options : dict
            ...
            schur : bool or 'auto', optional
                ...
                cover-layer faces from the final Sigma matrix.
                ``True`` / ``'auto'`` enables when a cover layer is
                detected; ``False`` (default) keeps the full BEM matrix.
                Only the dense path supports Schur; iterative
        ...
        # Optional Schur-complement reduction.
    ...
    def init(self, enei):
        ...
        # Distributed multi-GPU build: when MNPBEM_VRAM_SHARE_GPUS>=2
        ...
        # Schur option only supported on the CPU path today.
        ...
        # Route the heavy assembly through a cupy-eager fast path
        ...
        # Schur option currently only supported on the CPU path;
        ...
        def _to_dense(x):
            ...
            # If upstream Green-function returned a cupy array
            # coerce to host here.  The CPU init path
        ...
        # When the LU lives on GPU, build the identity RHS on the
        ...
        # layer face block of Sigma. When active, Sigma_lu factors
    ...
    def _init_gpu_assemble(self, enei):
        """Cupy-eager BEM matrix assembly.
        ...
        # per-wavelength fragmentation cleanup.  When the same
        ...
        #
        # held the high-water mark stable for ~10 wl on
        ...
        # Transfer to GPU.  The GPU Green-function path returns cupy
        ...
        # Upload the four host matrices for the on-device
        ...
        # The prior code set ``L1_is_scalar=True`` whenever
        ...
        cp.cuda.runtime.deviceSynchronize()
        _mempool.free_all_blocks()
        # Keep Sigma1 on device when GPU_NATIVE is enabled so the
        ...
        del Sc
        # the local lu/piv handles for G1/G2/Delta now alias the
    ...
    def _init_distributed_assemble(self, enei):
        """Distributed multi-GPU BEM matrix assembly.
        ...
        # Upfront cleanup: when the same BEMRet is reused across
    ...
    def solve(self, exc):
        ...
        # GPU_NATIVE — keep tensors on device end-to-end.
        # ``_bem_native_gpu()`` only reads the env var; it
        ...
        def _ls_sigma(b):
            # Wrapper that transparently applies the Schur-complement
        ...
        if npol == 1:
            ...
            # Multiple polarizations - vectorized over npol axis
        ...
        # Host-materialize cupy results before returning
