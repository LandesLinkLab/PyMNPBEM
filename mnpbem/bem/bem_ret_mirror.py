...
# cupy pool clean-up helpers mirroring bem_ret_layer.py
try:
...
def _mirror_eval_host(g: Any,
    ...
    code path.  Route the contraction through the host.
    """
    tab = g.p.symtable
    n_sym = tab.shape[0]
    out: List = [0.0] * n_sym
...
class BEMRetMirror(object):
    ...
    def __init__(self,
        ...
        # Distributed buffer caches.  Per-symmetry lists
    ...
    def init(self, enei: float) -> 'BEMRetMirror':
        ...
        # MATLAB-parity: drop previous wavelength's cached LU /
        ...
        for i in range(n_sym):
            ...
            sigma1 = H1[i] @ g1i
            sigma2 = H2[i] @ g2i
            # per-symmetry intermediates (g1i, g2i are full N^2
    ...
    def solve(self, exc: CompStructMirror) -> Tuple[CompStructMirror, 'BEMRetMirror']:
        ...
        for i in range(len(exc.val)):
            ...
            # Host-materialize so user code can call np.asarray.
        ...
        # free per-polarization LU scratch (cupy 2n RHS + GEMMs).
        _free_cupy_pool()
