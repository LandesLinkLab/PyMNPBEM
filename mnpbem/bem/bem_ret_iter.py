...
# GPU memory-pool cleanup: mirror BEMRet's wavelength-end immediate
...
def _gpu_pool_cleanup_iter(apply_limit: bool = False) -> None:
    ...
    Mirrors the BEMRet helper (bem_ret.py:485-503, 734-737).  The
...
def _gpu_precond_capacity_ok(N: int) -> bool:
    # decide whether the serialized GPU precond pipeline can fit.
...
def _build_precond_gpu_serial(
    ...
    # hybrid GPU LU + host matmul pipeline.
    #
...
class BEMRetIter(BEMIter):
    ...
    def __init__(self,
        ...
        # Schur option: cover-layer (EpsNonlocal) shell-face
        ...
        # added ``schur_eps_form`` to communicate to the
        ...
        # H-matrix: opt-in ACA acceleration of Green functions.
        ...
        # H-matrix LU preconditioner:
        #   'auto'      — pick dense for small mesh, tree for large
        #   'none'      — disable preconditioner entirely (legacy behaviour)
        ...
        # Default ``precond``: when the H-matrix path is active and
        # the user did not explicitly choose the legacy preconditioner, we
        # leave it disabled. The new H-matrix LU preconditioner is
    ...
    def _init_matrices(self,
        ...
        # wavelength-entry GPU cleanup.  Drop the previous
        ...
        if not distributed_built:
            ...
            # release the cross-block intermediates (Gxx) so the cupy
            ...
            # same logic for the H-block intermediates.
            del H11, H21, H22, H12
            if _CUPY_OK_ITER:
                _cp_iter.cuda.runtime.deviceSynchronize()
                _cp_iter.get_default_memory_pool().free_all_blocks()

            # When MNPBEM_GPU=1 + dense path, CompGreenRet returns
        ...
        if self.precond is not None:
            ...
            # the preconditioner builds up to 4 dense LUs and several
        ...
        # Schur: detect cover-layer partition and prepare the
        ...
        if self._schur_opt:
            ...
            if partition is not None:
                ...
                # resolve eps_form.  'auto' picks 'operator'
                # whenever ``_afun`` follows the operator-form path,
    ...
    def _compress(self,
        ...
        #
        # Note: HMatrix.full() auto-detects GPU blocks and would
        ...
        # Before the dense LU pipeline runs.  Force
    ...
    def _init_precond(self,
        ...
        #
        # non-uniform-eps fix.  When ``g.con[0][1]``
        ...
        # Coerce any cupy operands down to host before the
        ...
        # LU factorizations of Green functions.  12672-face note:
        ...
        # the 49 GB single-A6000 cap.
        #
        # Precond GPU path: we run a *hybrid* pipeline that
        ...
        if not use_gpu_serialized:
            ...
            # even on the host-LU fallback we want to drain the
        ...
        # is the operator-form Sigma1·L1 (= H1·eps1·G1⁻¹), used by
        ...
        sav['Sigma1_L1'] = Sigma_L1               # operator form: H1·eps1·G1⁻¹
        sav['L1'] = L1                            # G1·eps1·G1⁻¹ (or scalar)
        sav['L2'] = L2
        sav['Delta_lu'] = Delta_lu
        sav['Sigma_lu'] = Sigma_lu

        # optional GPU residency for the dense matrices
        ...
        # drain the cupy pool at the end of _init_precond so the
    ...
    def _build_distributed_GH(self,
            enei: float) -> None:
        """build G1/H1/G2/H2 with distributed assembly.
    ...
    def _gpu_mfun_enabled() -> bool:
        # Opt-in.  Default OFF leaves the host matmul path unchanged.
    ...
    def _afun(self,
        ...
        # Garcia de Abajo and Howie, PRB 65, 115418 (2002)
        #
        # non-uniform-eps fix.  When the particle has
        ...
        def _ensure_numpy(arr: Any) -> np.ndarray:
            # defensive wrapper for HMatrix matvec output.  After
            # the HMatrix fix the matvec result mirrors the input
    ...
    def _mfun(self,
        ...
        # Garcia de Abajo and Howie, PRB 65, 115418 (2002)
        #
        # non-uniform-eps fix.  Mirrors the dense
        ...
        # GPU-resident dense matrices (None when flag off).
        ...
        def matmul_op_disp(op_val: Any, op_gpu: Any, b: np.ndarray) -> np.ndarray:
            # GPU dispatch for L1 / L2 / L_diff.  Scalar op_val
    ...
    def solve(self,
        ...
        # drain right before GMRES enters its Krylov build-up so
        ...
        if self._schur_active:
            # Schur path: GMRES iterates on the reduced (core-only)
            ...
            # drop the Schur-reduced solution vector handle.
            del x_core
            if _CUPY_OK_ITER:
                _cp_iter.cuda.runtime.deviceSynchronize()
                _cp_iter.get_default_memory_pool().free_all_blocks()
            ...
            # H-matrix LU preconditioner.  Replaces fm
            ...
            # drop the GMRES matvec closures (which keep refs to
        ...
        # CPU-resident. Mirrors BEMRet.solve's defensive guard and lets
        # downstream code (sigma cache dump, field evaluator, surface
        # charge plotter) treat sig fields as numpy without per-call
        # cupy/numpy branching.
        ...
        # solve-exit cleanup: drain any residual transient buffers
    ...
    def _build_hlu_preconditioner(self,
        ...
        # Equivalent to ``precond='hmat'`` -- but now triggered on the
        # H-matrix code path where it was previously disabled.
        #
        ...
        # Trigger the dense initprecond path. This builds self._sav.
    ...
    def clear(self) -> 'BEMRetIter':
        ...
        self._H2 = None
        self._sav = None
        # explicit clear() means the user wants the device drained.
